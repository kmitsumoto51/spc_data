...
int main(int argc, char **argv){
  ...
  stringstream name1;
  name1 << "eq_physical" << "/" << L 
  << "_" << mu_in << "_" << Pre_in << "_" << alpha_in << "_" << k_in << "_" << step1 << "_" << seed << "_pair_pair_ref.txt";
  ...
  for(int p_d = 1; p_d < L/2; p_d++){
    ...
    for(int i=0; i<N; i++){
      if(surrounding[i]==1){
        sum_p4 += p_plaquette2[i];
      }
    }
    f1 << p_d << " " << E << " " << sum_p1 << " " << sum_p2 << " " << sum_p3 << " " << sum_p4 << endl;
    fstream f4;
    stringstream name4;
    name4 << "conf" << "/" << L << "_" << mu_in << "_" << Pre_in << "_" << alpha_in << "_" << k_in
     << "_" << step1 << "_" << seed << "_" << p_d << "_ref_pair_pair.txt";
