...
int main(int argc, char **argv){
  ...
  stringstream name1;
  name1 << "eq_physical" << "/" << L 
  << "_" << mu_in << "_" << Pre_in << "_" << alpha_in << "_" << k_in << "_" << step1 << "_" << seed << ".txt";
  ...
  while(T > T_finish+0.00001){
    ...
    for(int t=0; t<step1; t++){
    	metropolis_ads(sigma, position, n_b, alpha, k, mu, a1, beta);
    	for(int l=0; l<L; l++){
    		metropolis_position(sigma, position, n_b, alpha, k, a1, beta);
    	}
      metropolis_volume(sigma, position, n_b, alpha, k, mu, a1, Pre, beta);
    }
    for(int t=0; t<step2; t++){
      ...
      metropolis_volume(sigma, position, n_b, alpha, k, mu, a1, Pre, beta);
      Nads = num_ads(sigma);
      E = enthalpy(sigma, position, n_b, alpha, k, mu, a1, Pre);
      volu = a1*a1*L*L;
    	physical_q[0] += E;
    	physical_q[1] += E*E;
    	physical_q[2] += Nads;
