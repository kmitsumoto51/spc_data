...
int main(int argc, char **argv){
  ...
  stringstream name1;
  name1 << "eq_physical" << "/" << L 
  << "_" << mu_in << "_" << Pre_in << "_" << alpha_in << "_" << k_in << "_" << step1 << "_" << seed << "_high.txt";
  ...
  while(T > T_finish+0.00001){
    ...
    E_ave = physical_q[0]/(double)(step2/5);
    E_sq_ave = physical_q[1]/(double)(step2/5);
    C=specific_heat(E_sq_ave,E_ave,beta);
    Nads_ave = physical_q[2]/(double)(step2/5);
    Nads_sq_ave = physical_q[3]/(double)(step2/5);
    Nads_ave_sq = Nads_ave*Nads_ave;
    chi = susceptibility(Nads_sq_ave,Nads_ave_sq,beta);
    volume_ave = physical_q[4]/(double)(step2/5);
    volume_sq_ave = physical_q[5]/(double)(step2/5);
